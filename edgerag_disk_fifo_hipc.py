...
class ClusterCache:
    ...
    def evict_if_needed(self, new_cluster_count):
        ...
        if current_size + new_cluster_count > self.max_size:
            ...
            print(f"[FIFO needed Eviction] Removing {num_to_evict} entries.")

            eviction_candidates = list(self.cache.keys())[:num_to_evict]
    ...
    def put(self, cluster_id, embeddings, generation_latency):
        if cluster_id in self.cache:
            ...
            if len(self.cache) >= self.max_size:
                ...
                print(f"[FIFO Eviction] Removing Cluster {evicted_id}")
...
if __name__ == "__main__":
    ...
    result_filename = f"{timestamp}_nlist_{nlist_s}_cache_{cache_size_s}_thread_{num_thread}_fifo.csv"
    # [변경된 부분]
