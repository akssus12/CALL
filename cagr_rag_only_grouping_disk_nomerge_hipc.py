        ...
        if missing_clusters is not None:
            ...
            if len(missing_clusters) > 0:
                with ThreadPoolExecutor(max_workers=num_workers) as executor: # max_threads=10, min_shard_size=500, max_shard_size=2000)
                    ...
                    #futures = {executor.submit(wrapped_load_and_cache_cluster, cluster_id, self.cluster_embedding_path, self.cluster_generation_latency, self.cache): cluster_id for cluster_id in thread_assignments}
                    #futures = {executor.submit(load_and_cache_cluster_mmap, cluster_id, self.cluster_embedding_path, self.cluster_generation_latency, self.cache, max_threads=16, min_shard_size=512, max_shard_size=2048): cluster_id for cluster_id in thread_assignments}
                    # load_and_cache_cluster_parallel_read
                    #futures = {executor.submit(load_and_cache_cluster_parallel_read, cluster_id, self.cluster_embedding_path, self.cluster_generation_latency, self.cache, max_threads=1): cluster_id for cluster_id in thread_assignments}
