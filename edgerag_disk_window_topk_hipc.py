...
class ClusterCache:
    def __init__(self, max_size, window_duration = 60):
        self.cache = OrderedDict()  # Store embeddings {cluster_id: {"embeddings": np.array, "latency": float, "count": int}}
        self.max_size = max_size
        #----------EVCT 매트릭을 위한 변수 추가-----------
        # cache eviction 발생할 때마다 eviction++
        #--------------------------------------------
        self.cache_eviction = 0

        #----------window manager 변수 추가 -----------
        # window를 설정을 위한 변수
        #--------------------------------------------
        # Start background thread for window management
        self.window_duration = window_duration
        self.window_access_count = defaultdict(int)
        self.window_start_time = time.time()
        self.lock = threading.Lock()
        self.window_thread = threading.Thread(target=self._window_manager, daemon=True)
        self.window_thread.start()

    def _window_manager(self):
        # Background thread that manages window-based eviction every minute
        while True:
            time.sleep(self.window_duration)
            with self.lock:
                self._handle_window_expiry()

    def _handle_window_expiry(self):
        # Handle window expiry: keep only top-10 accessed clusters
        current_time = time.time()
        print(f"[Window Expiry] Processing window from {time.ctime(self.window_start_time)} to {time.ctime(current_time)}")

        if len(self.window_access_count) == 0:
            print("[Window Expiry] No access recorded in this window")
            self.window_start_time = current_time
            return

        # Sort clusters by access count in current window (descending)
        top_clusters = sorted(
            self.window_access_count.items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]  # Keep top 10

        top_cluster_ids = {cluster_id for cluster_id, count in top_clusters}

        print(f"[Window Expiry] Top 10 clusters in window: {dict(top_clusters)}")

        # Remove clusters not in top-10 from cache
        clusters_to_remove = []
        for cluster_id in self.cache.keys():
            if cluster_id not in top_cluster_ids:
                clusters_to_remove.append(cluster_id)

        for cluster_id in clusters_to_remove:
            del self.cache[cluster_id]
            print(f"[Window Expiry] Evicted Cluster {cluster_id}")

        print(f"[Window Expiry] Evicted {len(clusters_to_remove)} clusters, kept {len(self.cache)} clusters")

        # Reset window tracking
        self.window_access_count.clear()
        self.window_start_time = current_time

    def _calculate_eviction_priority(self, cluster_id):
        # Compute eviction priority for immediate eviction (when cache is full)
        cluster_data = self.cache[cluster_id]
        # Lower values indicate candidates for eviction
        # Consider both generation latency and recent access frequency
        recent_access_bonus = self.window_access_count.get(cluster_id, 0)
        return cluster_data["latency"] * cluster_data["count"] * (1 + recent_access_bonus)
    ...
    def evict_if_needed(self, new_cluster_count):
        ...
        if current_size + new_cluster_count > self.max_size:
            ...
            print(f"[Cache Top10 needed Eviction] Removing {num_to_evict} entries.")
        ...
        return self.cache_eviction

    def get(self, cluster_id):
        # Get cluster embeddings from cache
        with self.lock:
            current_time = time.time()
    ...
    def force_window_expiry(self):
        # Manually trigger window expiry (for testing purposes)
        with self.lock:
            self._handle_window_expiry()
...
class EdgeRAGWithCache:
    def __init__(self, ivf_centroids_path, cache_size, cluster_embedding_path, result_path):
        ...
        # ---------------------------------------
        # EVCT : the count of the cache eviction

        ############ 0620 nomerge index -> parallel indexing & search ############
        with open(self.result_path, mode='w', newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Seq", "Batch_Seq", "Duration", "EBT", "FLT", "CLT", "SLT", "NMC", "EVT", "PUT", "VST", "CHR", "QG", "NUM", "DRS"])
    
    def write_results_to_csv(self, seq, bs, tt, ebt, flt, clt, slt, nmc, evt, put, vst, chr, qg, num, drs):
        with open(self.result_path, mode="a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([seq, bs, tt, ebt, flt, clt, slt, nmc, evt, put, vst, chr, qg, num, drs])
        ############ 0620 nomerge index -> parallel indexing & search ############
    ...
    def search(self, query_text, model, sorted_duration, k, total_messages, dynamic_thread, num_thread):
        ...
        if missing_clusters is not None:
            ...
            # ------------- cache eviction 누적 수를 매 쿼리마다 기록하기------------
            self.cache_eviction_ct = self.cache.evict_if_needed(len(missing_clusters)) # Evict multiple entries if needed before inserting new ones
            ...
            if len(missing_clusters) > 0:
                ...
                print(f"[Query] File size for query {lookup_file_size}")
...
if __name__ == "__main__":
    ...
    result_filename = f"{timestamp}_nlist_{nlist_s}_cache_{cache_size_s}_thread_{num_thread}_window.csv"
    # [변경된 부분]
